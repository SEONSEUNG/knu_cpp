...
public:
    ...
    void save2csv() {
        ...
        for (const auto& row : clientVector) {
            outfile << row.number << "," << row.id << "," << row.pw << "," << row.name << "," << row.address << "," << row.phonenum << "," << row.hobby << "\n";
        }
        outfile.close();
    ...
    void addOneClient(int count) {
        ...
        cout << endl << "다음으로 넘어가려면 아무 숫자나 입력하세요: ";
        int vd;
        cin >> vd;
        cin.ignore();  // 여기서 버퍼 비우기 (getline 대비)
        cout << endl;
    }
    ...
    void addClient5() {
        ...
        cout << "-----------------------------" << endl
            << "클라이언트 추가 (5명)이 완료되었습니다." << endl
            << endl
            << "초기 화면으로 이동하려면 아무 숫자나 입력하세요: ";

        int vd;
        cin >> vd;
        cin.ignore(); // 이후 getline 대비 버퍼 정리
    }
